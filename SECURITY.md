...
- Recovery accepts only a plan ID from the server-owned, independently verified diagnosis report; clients cannot submit an arbitrary Bundle list. It can write only the selected profile manifest, verifies the diagnosis-time hash, creates an exclusive backup, and uses an atomic replacement. A restore refuses to overwrite later manifest changes.
